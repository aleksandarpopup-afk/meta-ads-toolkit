  ...
  try {
    if (req.method === "POST") {
      ...
      const r = await fetch(`${SUPABASE_URL}/rest/v1/ga4_connections`, {
        ...
        body: JSON.stringify({ user_id, client_id, property_id, property_name, refresh_token })
      });
    ...
    if (req.method === "GET") {
      ...
      const r = await fetch(
        `${SUPABASE_URL}/rest/v1/ga4_connections?client_id=eq.${client_id}&select=id,property_id,property_name,created_at`,
        { headers }
      );
      const data = await r.json();
